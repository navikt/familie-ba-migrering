...
class BekreftMigreringService(
    ...
    fun bekreftMigrering() {
        val saker = migrertsakRepository.findByStatus(MigreringStatus.MIGRERT_I_BA)
        LOG.info("Verifiser ${saker.size} migrerte saker")
        saker.forEach {
            bekreftMigrertSak(it)
        }
        val (sakerSuksess, sakerFeilet) = migrertsakRepository.findAllById(saker.map { it.id })
    ...
    fun bekreftMigrertSak(migrertsak: Migrertsak) {
        ...
        if (migreringResponseDto.infotrygdStønadId == null) {
            migrertsakRepository.update(migrertsak.copy(status = MigreringStatus.VERIFISERING_FEILET,
